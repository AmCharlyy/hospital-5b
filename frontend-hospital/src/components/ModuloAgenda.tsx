...
export function ModuloAgenda() {
  ...
  const [modalConfirmacion, setModalConfirmacion] = useState<{ isOpen: boolean; cita: any | null; accion: 'Confirmar' | 'Cancelar' | null }>({ isOpen: false, cita: null, accion: null });
  ...
  // Filtrado de citas
  const citasPendientes = citas.filter(c => c.estado === 'Pendiente').sort((a, b) => new Date(`${a.fecha}T${a.hora}`).getTime() - new Date(`${b.fecha}T${b.hora}`).getTime());
  
  let citasDelDia = citas.filter(c => c.fecha === fechaSeleccionada).sort((a, b) => a.hora.localeCompare(b.hora));
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-black/[0.05] shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-orange-50 flex items-center justify-center text-orange-500">
            <AlertCircle className="w-6 h-6" />
          </div>
          <div>
            <p className="text-sm font-medium text-[#86868b]">Pendientes (Total)</p>
            <p className="text-2xl font-semibold text-[#1d1d1f]">{kpis.pendientes}</p>
          </div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-black/[0.05] shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#86868b]">Confirmadas Hoy</p>
            <p className="text-2xl font-semibold text-[#1d1d1f]">{kpis.confirmadasHoy}</p>
          </div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-black/[0.05] shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#86868b]">Canceladas Hoy</p>
            <p className="text-2xl font-semibold text-[#1d1d1f]">{kpis.canceladasHoy}</p>
          </div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-black/[0.05] shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-[#86868b]">Total Citas Hoy</p>
            <p className="text-2xl font-semibold text-[#1d1d1f]">{kpis.totalHoy}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-black/[0.05]">
        <button
          ...
          className={`px-6 py-3 font-medium text-sm border-b-2 transition-colors ${vistaActiva === 'confirmaciones' ? 'border-blue-600 text-blue-600' : 'border-transparent text-[#86868b] hover:text-[#1d1d1f]'}`}
        >
          Confirmaciones Pendientes
        </button>
        <button
          onClick={() => setVistaActiva('calendario')}
          className={`px-6 py-3 font-medium text-sm border-b-2 transition-colors ${vistaActiva === 'calendario' ? 'border-blue-600 text-blue-600' : 'border-transparent text-[#86868b] hover:text-[#1d1d1f]'}`}
        >
                ...
                <div key={fecha} className="pb-4">
                  ...
                  <div className="divide-y divide-black/[0.05]">
                    {citasPendientesAgrupadas[fecha].map((cita) => {
                      ...
                      return (
                        <div key={cita.id_cita} className="p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:bg-gray-50/50 transition-colors">
                          <div className="flex items-start gap-4 flex-1">
                            <div className="flex flex-col items-center justify-center w-16 h-16 rounded-2xl bg-orange-50 text-orange-600 shrink-0">
                              <span className="text-xs font-medium uppercase">Hora</span>
                              <span className="text-lg font-bold leading-none">{cita.hora}</span>
                            </div>
                            <div>
                              ...
                              {paciente && (
                                <div className="flex items-center gap-3 mt-3">
                                  <button className="flex items-center gap-1.5 text-xs font-medium text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md hover:bg-blue-100 transition-colors">
                                    <Phone className="w-3.5 h-3.5" /> Llamar: {paciente.codigo_pais} {paciente.numero_telefono}
                                  </button>
                                  <button className="flex items-center gap-1.5 text-xs font-medium text-green-600 bg-green-50 px-2.5 py-1 rounded-md hover:bg-green-100 transition-colors">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-4 border-b border-black/[0.05] bg-gray-50/50 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 border-r border-black/[0.05] pr-3">
                ...
                <select 
                  className="bg-transparent text-sm font-medium text-[#1d1d1f] focus:outline-none"
                  value={filtroDoctor}
              ...
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className="px-3 py-1.5 bg-white rounded-lg text-sm font-medium text-[#1d1d1f] border border-black/[0.05] focus:outline-none focus:ring-2 focus:ring-blue-500/20 shadow-sm"
                />
              </div>
            </div>
          </div>
          ...
          <div className="divide-y divide-black/[0.05]">
            {citasDelDia.length === 0 ? (
              ...
              citasDelDia.map((cita) => {
                const paciente = getPaciente(cita.id_paciente);
                return (
                  <div key={cita.id_cita} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center gap-6">
                      <div className="flex flex-col items-center justify-center w-20 h-20 rounded-2xl bg-gray-50 text-gray-600 border border-black/[0.05]">
                        <Clock className="w-6 h-6 mb-1" />
                        <span className="text-sm font-bold tracking-tight">{cita.hora}</span>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`text-[11px] font-semibold uppercase tracking-wider px-3 py-1.5 rounded-lg border ${getEstadoColor(cita.estado)}`}>
                        {cita.estado}
                      </span>
                      <Boton variante="secundario" className="px-3 py-1.5 text-xs" onClick={() => setCitaSeleccionada(cita)}>Ver Detalles</Boton>
                    </div>
                  </div>
                );
              })
      ...
      <Modal isOpen={modalAbierto} onClose={() => setModalAbierto(false)} titulo="Programar Nueva Cita">
        <form onSubmit={handleRegistrarCita} className="space-y-4">
          <Select 
            label="Paciente" 
            required 
          ...
          >
            ...
            {pacientes.map(p => (
              <option key={p.id_paciente} value={p.id_paciente}>{p.nombre_paciente}</option>
            ))}
          </Select>
          ...
          <Select 
            ...
            onChange={(e) => setNuevaCita({...nuevaCita, id_doctor: e.target.value})}
          >
            <option value="">-- Seleccione un doctor --</option>
            {doctores.map(d => (
              <option key={d.id_doctor} value={d.id_doctor}>{d.nombre_doctor}</option>
            ))}
          </Select>

          <Select 
            label="Espacio / Consultorio (Opcional)" 
            value={nuevaCita.id_consultorio}
            onChange={(e) => setNuevaCita({...nuevaCita, id_consultorio: e.target.value})}
          >
            <option value="">-- Sin asignar --</option>
            {consultorios.map(c => (
              <option key={c.id_ui} value={c.id_ui}>{c.nombre}</option>
            ))}
          </Select>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-gray-50 border border-black/[0.05] text-gray-600 flex flex-col items-center justify-center">
                <Clock className="w-6 h-6 mb-1" />
                <span className="text-xs font-bold">{citaSeleccionada.hora}</span>
              </div>
              <div>
            ...
            <div className="bg-gray-50 p-4 rounded-2xl border border-black/[0.05] space-y-3">
              ...
              <div className="flex justify-between items-center pt-2 border-t border-black/[0.05]">
                <div>
                  ...
                  <p className="text-[#1d1d1f] font-medium mt-1">{citaSeleccionada.fecha}</p>
                </div>
              ...
              <div className="pt-2">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                  {historialPaciente.slice(0, 3).map(h => (
                    <div key={h.id_cita} className="flex justify-between items-center p-2 rounded-lg bg-gray-50 border border-black/[0.05] text-sm">
                      <div>
                        <p className="font-medium text-[#1d1d1f]">{h.fecha} <span className="text-[#86868b] font-normal">a las {h.hora}</span></p>
            ...
            <div className="flex flex-col gap-3 pt-4 border-t border-black/[0.05]">
              {citaSeleccionada.estado === 'Confirmada' && (
                <Boton onClick={handleIniciarConsulta}>Iniciar Consulta (Paciente en sala)</Boton>
              )}
              {citaSeleccionada.estado === 'En Curso' && (
                <Boton onClick={handleFinalizarConsulta}>Finalizar Consulta</Boton>
              )}
              
              <div className="flex gap-3">
                <Boton className="flex-1" variante="secundario" onClick={() => setCitaSeleccionada(null)}>Cerrar</Boton>
                {(citaSeleccionada.estado === 'Pendiente' || citaSeleccionada.estado === 'Confirmada') && (
                  <Boton className="flex-1" variante="peligro" onClick={() => setModalConfirmacion({ isOpen: true, cita: citaSeleccionada, accion: 'Cancelar' })}>
                    Cancelar Cita
                  </Boton>
                )}
              </div>
      ...
      >
        {modalConfirmacion.cita && (
          <div className="space-y-6">
            <div className={`p-4 rounded-xl border ${modalConfirmacion.accion === 'Confirmar' ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
              <p className={`text-sm font-medium ${modalConfirmacion.accion === 'Confirmar' ? 'text-green-800' : 'text-red-800'}`}>
                ¿Estás seguro que deseas {modalConfirmacion.accion?.toLowerCase()} la cita de <strong>{modalConfirmacion.cita.nombre_paciente || getPaciente(modalConfirmacion.cita.id_paciente)?.nombre_paciente}</strong> programada para el <strong>{modalConfirmacion.cita.fecha}</strong> a las <strong>{modalConfirmacion.cita.hora}</strong>?
              </p>
            </div>
