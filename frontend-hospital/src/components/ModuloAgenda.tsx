...
export function ModuloAgenda() {
  ...
  // Normalizador de fechas para evitar problemas con T00:00:00.000Z
  const normalizarFecha = (fechaOriginal: string) => fechaOriginal ? fechaOriginal.split('T')[0] : "";
  // Normalizador de hora para quitar los segundos (10:00:00 -> 10:00)
  const formatearHora = (horaOriginal: string) => horaOriginal ? horaOriginal.slice(0, 5) : "--:--";

  // Filtrado de citas con fechas normalizadas
  ...
  const handleRegistrarCita = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nuevaCita.id_paciente) return; // Doctor y consultorio pueden ser nulos al registrar "Pendiente"
  ...
  // Helpers con manejo robusto para valores NULL
  const getPaciente = (id: string | number) => pacientes.find(p => String(p.id_paciente) === String(id));
  
  const getNombreDoctor = (id?: string | number | null) => {
    if (!id) return "Doctor por asignar";
    return doctores.find(d => String(d.id_doctor) === String(id))?.nombre_doctor || "Doctor no encontrado";
  };
  
  const getNombreHabitacion = (id?: string | number | null) => {
    if (!id) return "Espacio por asignar";
  ...
  // Agrupar usando fecha normalizada
  const citasPendientesAgrupadas = citasPendientes.reduce((acc, cita) => {
    const fechaLimpia = normalizarFecha(cita.fecha);
    if (!acc[fechaLimpia]) acc[fechaLimpia] = [];
    acc[fechaLimpia].push(cita);
    return acc;
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                            ...
                            <div className="flex items-start gap-4 flex-1">
                              ...
                              <div>
                                <div className="flex items-center gap-2 mb-1">
                                  <h3 className="text-lg font-semibold text-[#1d1d1f]">{cita.nombre_paciente || paciente?.nombre_paciente || "Paciente no encontrado"}</h3>
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-[#1d1d1f] flex items-center gap-2">
                            {cita.nombre_paciente || paciente?.nombre_paciente || "Paciente no encontrado"}
                          ...
                          <div className="flex items-center gap-4 mt-2 text-sm text-[#86868b]">
                            <span className="flex items-center gap-1"><User className="w-4 h-4" /> {getNombreDoctor(cita.id_doctor)}</span>
                            <span className="flex items-center gap-1"><Activity className="w-4 h-4" /> {cita.tipo_cita}</span>
                            <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {getNombreHabitacion(cita.id_consultorio)}</span>
                          </div>
                        </div>
                      </div>
      ...
      <Modal isOpen={modalAbierto} onClose={() => setModalAbierto(false)} titulo="Programar Nueva Cita">
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            <Select label="Doctor Asignado" value={nuevaCita.id_doctor} onChange={(e) => setNuevaCita({...nuevaCita, id_doctor: e.target.value})}>
          ...
          <motion.div variants={itemVariants} className="pt-4 flex justify-end gap-3">
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Boton type="submit" disabled={!nuevaCita.id_paciente}>Guardar Cita</Boton>
            </motion.div>
          </motion.div>
        </motion.form>
      </Modal>
        ...
        {citaSeleccionada && (
          <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
            <motion.div variants={itemVariants} className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-semibold text-[#1d1d1f]">{citaSeleccionada.nombre_paciente || getPaciente(citaSeleccionada.id_paciente)?.nombre_paciente || "Paciente no encontrado"}</h3>
